...
while (fgets(line, MAX_LINE, data) != NULL)
{
	...
	/* **BUG THIS DOESN't WORK - core dump if fasta format doesn't have a title */
...
rootres *read_file_quick(int num, int mask, off_t len)
{
char modifier[20];
rootres *create_root_residue(rootres *);
rootres *root;
...
/* set newp to beginning of file */
/*
mutex_lock(&mapfile_lock);
*/
/* Somebody else might have got there first */
if (end_of_file == 1) return((rootres *) NULL);
if (gptr == NULL) gptr = src;

newp = gptr;
if ((off_t)newp >= ((off_t)src + len)) return((rootres *)NULL);
while (*newp != '>') newp++;
if (newp == (char *) NULL)
{
...
/* Get the code in the FASTA format: >code */
i = 0;
newp++;
while(*newp != ' ')
{
...
if (root->code != NULL)
{
	strncpy(root->code, tempcode, strlen(tempcode));
	root->code[i] = '\0';
}
else
{
...
/* Save the rest of the title line */
while(*newp == ' ') newp++; /* skip the white space */
i = 0;
while (*newp != '\n')
{
	tempcode[i++] = *newp++;
...
/* Next line should be start of sequence */

start_seq = newp; /* Save start of sequence */
cgap = 0;
cres = 0;
cjunk = 0;
...
gptr = newp;
/*
mutex_unlock(&mapfile_lock);
*/

root->seq = (char *)calloc(cres+3, sizeof(char));
if (root->seq == NULL)
{
	printf("*** calloc failed in root->seq\n");
	exit(0);
}
root->gap = (int *)calloc(cres+3, sizeof(int));
if (root->gap == NULL)
{
	printf("*** calloc failed root->gap\n");
	exit(0);
...
newp = start_seq; /* Go back and copy the sequence */
root->assoc = (pres *)NULL;
numres = 0;
root->numres = 0;
root->numgap = 0;
...
while (*newp != '>')
{
	...
	else 
	{
		if (TRANSLATION == TRUE)
		{
			if (strchr(gcode_template, *newp) == NULL) root->seq[numres] = 'N';
			else root->seq[numres] = *newp;
		}
		else
		{
			root->seq[numres] = *newp;
		}
		numres++;
	}
	newp++;
