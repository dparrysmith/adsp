...
printf("Features: multithreaded, memory mapped databases, statistics.\n");
if (argc == 1)
{
...
/* 5/96 initialise stats buffer */
for (i = 0; i < 100; i++)
{
	feature[f_ind]->stats[i] = 0;
}
...
if ((log_data = fopen("prints.stats","w")) == NULL)
{
	fprintf(stderr," - could not open file 'prints.stats'.");
	perror("prints.stats");
	exit(1);
}
...
f_ind = 1;
/* 5/96 BUG fixed <= MAX_FEATURES goes right to end of feature list */
while(f_ind <= MAX_FEATURES && feature[f_ind] != NULL)
{
int prints_code;

/* first calculate stats on this motif scan */
/* djps: 5/96 */
for (istat = 0; istat < 100; istat++)
{
	s += feature[f_ind]->stats[istat];
}
ave = s/100.0;
adev = 0.0;
var = 0.0;
...
for (istat = 0; istat < 100; istat++)
{
	s = feature[f_ind]->stats[istat] - ave;
	adev += (float)fabs((double) s);
	p = s * s;
	var += p;
	p *= s;
	skew += p;
...
if (var != 0.0)
{
	skew /= (100.0 * (float)pow((double)sdev, 3.0));
	curt /= (100.0 * (float)pow((double)var, 2.0));
}
	...
	while (current != NULL)
	{
		...
		else
		{
			...
			if (prints_code != 1)
			{
				fprintf(hit_data,"\n\n%s\n",feature[f_ind]->code);
/*
				fprintf(hit_data,"Stats: ave = %f, sdev = %f, skew = %f, curtosis = %f\n", ave, sdev, skew, curt);
*/
				/* write stats data to a log file */
				fprintf(log_data,"%s\t", feature[f_ind]->code);
				for (istat = 0; istat < 100; istat++)
				{
					fprintf(log_data,"%d\t", feature[f_ind]->stats[istat]);
				}
				fprintf(log_data, "\n");
				prints_code = 1;
			}
...
/*
		fprintf(hit_data,"%4d) %6.2f  %6.2f (%6.2f)  %-14.12s%4d - %4d   %s\n",i++,current->score,
		current->score - ave,
		(current->score - ave)/sdev,
...
*/
		...
		{
/*
		fprintf(hit_data,"%4d) %6.2f %6.2f (%6.2f)   %-16.14s%5d - %5d %2d %s\n",
			i++,
			current->score,
			current->score - ave,
			(current->score - ave)/sdev,
...
while (f_ind < MAX_FEATURES && feature[f_ind] != NULL)
{
	...
	*anchor_score *= num_matches; /* Special scan_novel modification */
	...
	{
		/* 5/96: djps modified to use a separate lock on each hitlist so that merging can multi-process */
		mutex_lock(&feature[f_ind]->mlock);
		merge_hits(mine, f_ind);
		update_stats(mine, f_ind);
...
void update_stats(rootres *mres, int f_ind)
{
int i;
/* This proceudre chops the floating point value to the lower integral
value. This is ok for jazz but may need more rigour for publication */

for (i = 0; i < mres->numres; i++)
{
	feature[f_ind]->stats[(int)mres->fdata[i]]++;
}
return;
}/* End of update_stats */ 
