/*
Title           : SCAN_COMMAND
Module          : scp_new.c was SCAN_COMMAND.C
Version         : 2.0
Author          : D.J.Parry-Smith
Date            : 6-JUNE-1989
History         : Bugs fixed to 27-AUG-1996
		  Incorporates plot as well as scan.
		  Segment input is ok.
		  Incorporates translation of DNA
		  Support for searching PRINTS format databases added 2/95
		  New scoring scheme for PRINTS implemented 8/96
Description
...
#define ADSP_VERSION "v3.2"
#define DCL_STRING 256
#define NCPU 8
#define MAX_FEATURES 3300
...
/* Create a summary based on each fingerprint.
The scoring scheme is: 1/Si * 1/Sj * 1/Sk ... So the Log10 of the score gives a measure of confidence
in the proposition that the hit has occurred by chance.
DJPS August 1996
*/

/* First need to determine for each print how many elements there are as this information is not
stored from the PRINTS database.
*/
...
int finito;
	...
	/* printf("%s %d\n", feature[f_ind]->gc, feature[f_ind]->elements); */


	/* This is a niaive algorithm for matching over the hitlists */
	/*
		start with hitlist f_ind and search each other list for the code
		perform the scoring algorithm

		Note for this release we only take the bestscoring hit for each code from the
		hitlist.

		The search is not well implemented and is better in compare.
	*/
	fprintf(hit_data,"\n\n%s\n", feature[f_ind]->gc);

	f_ind_1 = f_ind + feature[f_ind]->elements;
	

	for (fi = f_ind; fi < f_ind_1; fi++)
	{
	char *temp_code;
	double new_score;
	double logged;
		...
		hlist *nc;
			...
			/* Store the code */
			temp_code = current->code;
			new_score = 1.0/current->score;
			for (fii = fi + 1; fii < f_ind_1; fii++)
			{
				/* now search in the other hitlists */
				finito = 0;
				nc = hits[fii];
				if (nc->code[0] == '\0') break;
				while (nc != NULL)
				{
					if (nc->code[0] == '\0') break;
					if (strcmp(temp_code, nc->code) == 0)
					{
						/* Ignore spacing for now and go for the score */
						new_score *= 1.0/nc->score;
						finito = 1;
					}
					if (finito = 1) break;
					else nc = nc->next;
		...
		/* Print out the code and its combined score */
logged = log10(new_score)/(double)feature[f_ind]->elements;
		if (logged < -1.0 )
		{
			fprintf(hit_data,"%-14.12s  %6.2e\t%6.3f\n", temp_code, new_score, logged);
		}
		current = current->next;
		}/* End while current... */
	} /* Next fi */
...
f_ind = f_ind_1; /* Set the main feature index pointer. */
} /* End while */
...
/*Comments aren't very liberal here - you will need to look in my
thesis for a description of what these scanning methods actually do and
how they score matches.
*/
...
while (feof(in_data) == 0 )
{
	while (fgets(line, MAX_LINE, in_data) != NULL)
	{
		/* August 1996: Record data on fingerprint name 'gc' and number of elements */
		if (strncmp(line, "gc;", 3) == 0)
		{
			/* Store code data temporarily and copy to feature structure when created */
			strncpy(gc, &line[4], MAX_PRINTS_CODE_LEN);
			while (fgets(line, MAX_LINE, in_data) != NULL)
			{
				if (strncmp(line, "gn;", 3) == 0)
				{
					/* Get the number of elements */
					if (sscanf(line, "gn; COMPOUND(%d", &elements) == 0)
					{
						elements = 1;
					}
					break;
		...
		else if (strncmp(line, "fc;", 3) == 0) break; /* Go on to get the segments */
	}
	...
	feature[f_ind] = (fth *) malloc(sizeof(fth));
	demand(feature[f_ind], malloc failed in read_segments);
	strcpy(feature[f_ind]->file_spec, "PRINTS"); /* Version information ? */
	strcpy(feature[f_ind]->gc, gc); /* Copy the general code */
	feature[f_ind]->elements = elements;
