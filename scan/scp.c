...
rootres *read_file_quick(int, int, int);
...
struct stat statbuf; /* for the fats db reading */
...
char *p;
	...
	for (th = 0; th < MAX_THREADS; th++)
	{
		...
		if (error != 0)
		{
			fprintf(stderr,"thr_create returned non-zero\n");
			perror("thr_create");
			exit(error);
		}
	}
...
if (val != 0)
{
	...
	strcpy(inserted->code,mres->code);
 	strncpy(inserted->comment,mres->comment,80);
...
#else
if ((mine = read_file_quick(din, block, scan_modifier)) == NULL) ret_flag = 1;
#endif
